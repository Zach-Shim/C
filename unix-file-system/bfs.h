...
typedef struct {          // Open File Table Entry
  i32 inum;               // inum of file. O => slot not used; inum = {0-16}
  i32 refs;               // # processes fsOpen'd this file
  i32 curs;               // cursor (seek pointer) into file
} OFTE;

OFTE g_oft[NUMOFTENTRIES];

i32 bfsExtend(i32 inum, i32 fbn);         // used for fswrite if you want to add another FBN   
...
i32 bfsInitOFT();                        // CALLED FROM MAIN
...
i32 bfsFbnToDbn(i32 inum,   i32 fbn);       // called by bfsRead
