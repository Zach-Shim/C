...
// ==============================================================================================
// Extend file 'inum' out to FBN 'fbn'; From (old largest FBN+1) to (new largest FBN in param)
// ==============================================================================================
...
i32 bfsAllocBlock(i32 inum, i32 fbn) {
  ...
  if (fbn < NUMDIRECT) {                  // in direct[] array?
    ...
    i16 buf16[I16SPERBLOCK]= {0};         // declare buffer the size of an index block
    i32 dbnIndirect = pinode->indirect;   // DBN of indirect block

    if (dbnIndirect == 0) {               // indirect block has not yet allocated
      dbnIndirect = bfsFindFreeBlock();   // get a free block
      pinode->indirect = dbn;             // get the DBN of the free block
    }

    bioRead(dbnIndirect, buf16);          // read single indirect index(pointer) block into memory

    buf16[fbn - NUMDIRECT] = dbn;         // index in the index block we want to write to
    bioWrite(dbnIndirect, buf16);         // write all zeroes to the free block
  }
...
// if not yet mapped; Helper for bfsread
// ============================================================================
i32 bfsFbnToDbn(i32 inum, i32 fbn) {
  ...
  // caller to handle grabbing a new data block.

  if (inode.indirect == 0) {        // no indirect block yet allocated
    i32 dbn = bfsFindFreeBlock();   // allocates the index block for the single indirect block
    inode.indirect = dbn;
